  ...
  using SafeERC20 for IERC20;
    ...
    function _checkpoint(
        ...
        if (addr != address(0)) {
            ...
            userPointEpoch[addr]++; //= userPointEpoch[addr] + 1/*userEpoch*/;
    ...
    function totalSupplyAt(uint256 _block) external view returns(uint256) {
        ...
        if (targetEpoch < _epoch) {
            ...
            //!u targetEpoch
            //!u _epoch
            //! pointHistory[targetEpoch].blk
            //!s pointHistory[targetEpoch].bias
            //!s pointHistory[targetEpoch].slope
            {
              // AUTOGENERATED
              // DELETE THIS BLOCK IN PRODUCTION IF IT IS STILL HERE AFTER TESTS:
              revert(
                  StringsConcatenations.strConcat(
                    StringsConcatenations.strConcat(
                      StringsConcatenations.uint2str(targetEpoch),
                      " ",
                      StringsConcatenations.uint2str(_epoch)
                    ),
                    " ",
                    StringsConcatenations.uint2str(pointHistory[targetEpoch].blk),
                    StringsConcatenations.strConcat(
                      StringsConcatenations.int2str(pointHistory[targetEpoch].bias),
                      " ",
                      StringsConcatenations.int2str(pointHistory[targetEpoch].slope)
                    )
                  )
              );
            }
