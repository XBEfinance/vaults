...
contract('BankV2', (accounts) => {
  ...
  describe('redeemBond', () => {
    ...
    it('Ok: redeeming EURxB and EURxC bonds as an owner', async () => {
      ...
      let vaultBalance = await this.bankV2.balanceOf.call(this.vault.address);
      ...
      const xbEURO = await this.bankV2.xbEUROvault.call(client, { from: client });
      console.log('Client, xbEURO balance (stored in vault): ', xbEURO.toString());

      // withdraw xbEURO
      await this.bankV2.withdraw(ether('90'), { from: client });
      vaultBalance = await this.bankV2.balanceOf.call(this.vault.address);
      console.log('Vault Balance after withdrawing 90 eth: ', vaultBalance.toString());
