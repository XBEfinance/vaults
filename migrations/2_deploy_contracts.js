...
const saveAddresses = () => {
  const jsonAddressData = JSON.stringify({
    ...
    hiveVault: wrapAddress(contracts['Vault']),
    cvxStrategy: wrapAddress(contracts['cvxStrategy']),
    cvxVault: wrapAddress(contracts['Vault']),
    cvxCrvStrategy: wrapAddress(contracts['cvxCrvStrategy']),
    cvxCrvVault: wrapAddress(contracts['Vault']),
  });
...
const deployContracts = async (deployer, owner) => {
  const deploySingleContract = async (contractName, artifact) => {
    ...
    if (needToDeployContract) {
      ...
      if (contractName === 'hiveVault' || contractName === 'cvxVault' || contractName === 'cvxCrvVault') {
        contracts[contractName] = await deployer.deploy(artifact, 'XBE Hive Curve LP', 'xh', { from: owner });
      } else {
